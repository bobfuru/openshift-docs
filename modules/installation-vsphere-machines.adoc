...
. Locate the bootstrap Ignition config file `<installation_directory>/bootstrap.ign` that the installation program created.

. Store the Ignition config file in the extra configuration properties of the virtual machine (VM). For example:
+
[source,text]
----
resource "vsphere_virtual_machine" "vm" {
  ...
  extra_config = {
    "<installation_directory>/bootstrap.ign"   <1>
  }
}
----
+
<1> Specify the installation directory of the bootstrap Ignition config file that you hosted.
...
----
+
... Set the extra config properties of the control plane, compute, and bootstrap Ignition config files before booting a VM from an OVA in vSphere. For example, to change the bootstrap Ignition config:
+
[source,terminal]
----
$ govc vm.change -vm "<vm_name>" -e "guestinfo.bootstrap.ign=${IPCFG}"
----
+
